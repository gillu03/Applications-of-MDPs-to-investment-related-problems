...
portfolio_results_PCI$`Total Consumption` <- cumsum(portfolio_results_PCI$`Optimal Consumption`)
...
plot_wealth_cons <- portfolio_results_PCI %>%
  select(Time, Wealth, `Total Consumption`) %>%
  pivot_longer(cols = c(Wealth, `Total Consumption`), names_to = "Variable", values_to = "Value")

# Plot
ggplot(plot_wealth_cons, aes(x = Time, y = Value, color = Variable)) +
  geom_line(linewidth = 1.2) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Wealth and Cumulative Consumption Over Time",
    x = "Time Step",
    y = "Value"
  ) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual(values = c("blue", "red"))  # Custom colors for the two lines
...
# Plot Wealth Process
ggplot(portfolio_results_ECI, aes(x = Time, y = Wealth)) +
  geom_line(color = "blue", linewidth = 1.2) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Wealth Process Over Time - Exponential Utility Consumption-Investment",
    x = "Time Step",
    y = "Wealth"
  ) +
  theme(plot.title = element_text(hjust = 0.5))
...
ggplot(portfolio_results_ECI, aes(x = Time, y = `Optimal Consumption`)) +
  geom_line(color = "red", linewidth = 1.2) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Optimal Consumption Over Time - Exponential Utility Consumption-Investment",
    x = "Time Step",
    y = "Consumption"
  ) +
  theme(plot.title = element_text(hjust = 0.5))
